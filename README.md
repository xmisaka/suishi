...
四屏主标签（星盘 / 全部 / 记一笔 / 我的），外加详情、编辑、回收站、备份与恢复、关于几个从属页。

下面六屏是 **v1.2.0 的实际渲染**，不是设计稿：照 `src/` 的源码逐屏重建，文案、色值、字号、
间距都取自实现，样本条目的落点由 `src/lib/calendar/resolve.js` 现算 ——
所以图上的「12 天后」「农历八月廿五」是算出来的。
源文件与重建方法见 [`docs/screens.html`](docs/screens.html)。

<p align="center">
  <img src="docs/screenshots/01-dial.png" width="246" alt="星盘 · 首页" />
  <img src="docs/screenshots/02-list.png" width="246" alt="全部 · 按「还有多久」分段" />
  <img src="docs/screenshots/03-detail.png" width="246" alt="条目详情" />
</p>

- **星盘** —— 一屏只做一件事：把一年摊成一张圆盘。盘外那圈就是时间，光点就是日子。
- **全部** —— 不按创建时间排，按**时间距离**分段，一眼知道该往下翻多远。
- **详情** —— 大号倒计时占住上半屏，下面才是事实。公历与农历**两个都列出来**。

<p align="center">
  <img src="docs/screenshots/04-compose.png" width="246" alt="记一笔" />
  <img src="docs/screenshots/05-mine.png" width="246" alt="我的" />
  <img src="docs/screenshots/06-theme.png" width="246" alt="主题面板" />
</p>

- **记一笔** —— 只问三件事：叫什么、哪天、怎么重复。其余不抢视线。
- **我的** —— 一屏装完「这台设备上怎么看」与「这份数据怎么保」。四个数字用来核对数据还在。
- **主题面板** —— 四套主题做成带色块的长列表：差别主要在「底色有多暗、金是暖还是冷」，
  摊开两个色块比只写名字选得准得多。
...
- **下面两段** —— 计数（今天 / 将到 / 还早）用来核对盘上没有漏画；
  「接下来的日子」列出最近五个。圆盘回答「一年有多满」，这一段回答「最近是谁」。
- 空白状态**不藏圆盘**：空盘本身就是引导，换成一个居中插画会把「这里将来会有一张盘」的预期抹掉。

### 二 · 记一笔

这个应用唯一的核心动作。表单只问三件事：**叫什么、哪天、怎么重复**。
...
分段表：今天 / 一周之内 / 一个月内 / 三个月内 / 一年之内 / 一年之后 / 已经过去。

### 四 · 我的

一屏装完所有「关于这台设备上怎么看」与「关于这份数据」的事，分四组：
概览（日子 / 农历 / 盘上 / 置顶，用来核对数据还在）／外观（主题）／
数据（回收站、备份与恢复）／关于（版本、来由）。
...
| 主题 | 说明 |
|---|---|
| **玄夜** `xuanye` | 墨底暖金，最像旧历书。**默认** |
| **苍青** `cangqing` | 墨蓝月白，夜最浓的一套 |
| **墨玉** `moyu` | 墨绿玉色，安静得最彻底 |
| **素笺** `sujian` | 暖米白配赭石棕，给白天的备选 |
...
scripts/                     校验、出图、构建
  lib/dial-svg.cjs           盘面 SVG 装配（出图脚本与六屏页面共用）
  screens-page.cjs           照 src/ 重建六屏 → docs/screens.html
docs/                        设计稿、方案页、界面截图（README 的图都在这里）
...
# 重建 README 用的六屏页面（图是它的渲染产物，改完要重出）
node_modules/.bin/tsc -p scripts/tsconfig.caltest.json && node scripts/screens-page.cjs
python ~/.workbuddy/skills/readme-product-screens/scripts/export_screens.py \
  docs/screens.html docs/screenshots
```
...
<p align="center">
  <img src="docs/岁时-月轮-玄夜特写.png" width="420" alt="天象盘 · 玄夜 · 月轮特写（900px 出图核对）" />
</p>
...
当前 **v1.2.0**（versionCode 3）。安装包在 [Releases](https://github.com/xmisaka/suishi/releases)。
...
三版共用同一把签名密钥，可直接覆盖安装。回退到更低 versionCode **必须先卸载**。
